...
async function insertRowWithoutReturn(tx: RawClient, tableName: string, row: DbRow) {
  const entries = Object.entries(row);
  const columns = entries.map(([key]) => quoteIdentifier(key)).join(", ");
  const placeholders = entries.map((_, index) => `$${index + 1}`).join(", ");
  const values = entries.map(([, value]) => value);
...
async function cloneRowsWithId(
  ...
  for (const row of rows) {
    const createdId = await insertRow(tx, tableName, buildRow(row), idColumn);
...
async function cloneTemplateCenter(tx: RawClient) {
  // Role: Clone one random seeded hospital into an isolated demo hospital dataset.
  ...
  const bedIdMap = await cloneRowsWithId(
    ...
    beds,
    (bed) => ({
      ...without(bed, ["bed_id", "room_id", "assigned_patient_id", "assigned_nurse_id"]),
...
export async function createDemoSession() {
  // Role: Create a brand-new demo session bound to a freshly cloned hospital.
  return prisma.$transaction(
    async (tx) => {
      const clonedCenter = await cloneTemplateCenter(tx);
...
export async function rotateDemoSessionHospital(sessionId: string) {
  // Role: Swap an existing demo session to a newly cloned random hospital.
  return prisma.$transaction(
    async (tx) => {
      await ensureDemoSchema(tx);

      const [session] = await tx.$queryRawUnsafe<Array<{ center_id: number }>>(
        `SELECT "center_id"
         FROM "demo_session"
         WHERE "session_id" = $1
           AND "expires_at" > NOW()
      ...
      // Role: Keep prior demo center rows to avoid foreign-key violations during rotation.
      // Cleanup can be handled by a dedicated janitor job that deletes dependent rows in order.
